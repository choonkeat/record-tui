...
func RenderStreamingPlaybackHTML(opts StreamingOptions) (string, error) {
	...
	// Always use large scrollback to ensure no content is clipped
	scrollback = 1000000

	if opts.EstimatedRows > 0 {
		// Caller provided content-based estimate - use it as initial viewport
		rows = opts.EstimatedRows
		autoResizeEnabled = true // Still auto-resize (but only shrink)
	} else if opts.Rows > 0 {
		rows = uint32(opts.Rows)
		autoResizeEnabled = true
	} else {
		// No hints - use reasonable default and auto-resize
		rows = 1000
		autoResizeEnabled = true
	}
...
` + js.CleanerCoreJS + `
    ...
    async function main() {
      ...
      try {
        ...
        // Only resize DOWN if content is smaller than initial allocation
        // This matches embedded template behavior - never grow, only shrink
        if (AUTO_RESIZE) {
          setTimeout(function() {
            const buffer = xterm.buffer.active;
            if (!buffer) return;

            // Find last row with content (search from end, limited range)
            const MAX_SEARCH = 10000;
            const searchStart = Math.max(0, buffer.length - MAX_SEARCH);
            ...
            // Only resize if content is SMALLER than initial rows (shrink, don't grow)
